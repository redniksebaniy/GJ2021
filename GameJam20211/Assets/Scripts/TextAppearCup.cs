...
{
    static public bool existanceC = true; //М: Тут помечаем существует ли объект на данный момент
    ...
    {
        if (existanceC) //М: Если он есть
        {
            target.SetActive(false);
            existanceC = false; //М: Выключаем объект и меняем bool
        }
        else
        {
            target.SetActive(true);
            existanceC = true; //М: Если нет то включаем его соответсвенно
        }
    }

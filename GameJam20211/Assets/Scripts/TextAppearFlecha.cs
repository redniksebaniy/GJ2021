...
{
    static public bool existanceF = true; //М: Тут помечаем существует ли объект на данный момент
    ...
    {
        if (existanceF) //М: Если он есть
        {
            target.SetActive(false);
            existanceF = false; //М: Выключаем объект и меняем bool
        }
        else
        {
            target.SetActive(true);
            existanceF = true; //М: Если нет то включаем его соответсвенно
        }
    }

...
{
    static public bool existanceH = true; //М: Тут помечаем существует ли объект на данный момент
    ...
    {
        if (existanceH) //М: Если он есть
        {
            target.SetActive(false);
            existanceH = false; //М: Выключаем объект и меняем bool
        }
        else
        {
            target.SetActive(true);
            existanceH = true; //М: Если нет то включаем его соответсвенно
        }
    }
